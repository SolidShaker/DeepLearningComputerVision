...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = torch.sum(
                (x_train_flat[i] - x_test_flat[j]) ** 2
            )
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        differences = x_test_flat - x_train_flat[i] 
        dists[i] = torch.sum(differences ** 2, dim = 1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_squared = torch.sum(x_train_flat ** 2, dim=1)
    test_squared = torch.sum(x_test_flat ** 2, dim=1)

    product = x_train_flat @ x_test_flat.T

    dists = train_squared[:, None] + test_squared - 2 * product
